...
public class ArtistCategoryDataThread extends Thread {

	private Handler handler;
	private List<CategoryListInfo> list;
	private static ArtistCategoryDataThread intance;

	private ArtistCategoryDataThread(Handler handler,
	...
	public static void startArtistCategoryDataThread(Handler handler,
			List<CategoryListInfo> list) {
		if (intance == null) {
			intance = new ArtistCategoryDataThread(handler, list);
			intance.start();
		}
	}
	...
	public void run() {
		...
		String datastr = null;
		try {
			url = new URL(Constants.API_CATEGORY_2012);
